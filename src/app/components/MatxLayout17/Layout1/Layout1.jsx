...
const Layout1 = () => {
  ...
  const verify_class = async (id) => {
    ...
    const result = await axios.post(
      process.env.REACT_APP_BACKEND_URL + 'admin/update/status',
      {
        classroom_id: id,
        status: 1,
      },
  ...
  const update_credit = async (id) => {
    ...
    const result = await axios.post(
      process.env.REACT_APP_BACKEND_URL + 'admin/update/price',
      {
        classroom_id: id,
        credit: price,
      },
  ...
  useEffect(async () => {
    ...
    const result = await axios.post(
      process.env.REACT_APP_BACKEND_URL + 'admin/fetch/classroom',
      {},
      {
        headers: headers,
      }
    ...
    const item1 = classroom.map((d) => {
      return (
        <tr className="inner-row">
          ...
          <td>{d.status == 1 ? 'verified' : 'not'}</td>
          <td>{d.credit}</td>
          <td>
            <button
              className={'edit-btn'}
              onClick={() => {
                verify_class(d.classroom_id);
              }}
            >
              verify
            </button>
