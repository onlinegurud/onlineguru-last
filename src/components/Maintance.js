...
const Maintance = () => {
  ...
  useEffect(async () => {
    const result2 = await axios.post('https://onlineguru.app/student/fetch', {});
  ...
  const remove_item = async (id, Name, quantity, Price) => {
    const result2 = await axios.post('https://onlineguru.app/student/update', {
      id: id,
      Name: Name,
      quantity: quantity,
      rating: 0,
    ...
    if (d.rating != '0') {
      ...
      return (
        <tr className="inner-row">
          <td>
            {d.id}
            <div className="unused-layer"></div>
          </td>
          <td>{d.Name}</td>
          <td>{d.quantity}</td>
          <td>{d.Price}</td>
  ...
  const f_submit_2 = async (event) => {
    ...
    const result2 = await axios.post('https://onlineguru.app/student/add', {
      id: event.target.id.value,
      Name: event.target.Name.value,
      quantity: event.target.quantity.value,
      rating: 1,
        ...
        <div className="popup-layer-2">
          <form className="form-maintance" onSubmit={f_submit}>
            ...
            <input
              ...
              value={i_id}
              onChange={(event) => {
                update_i_id(event.target.value);
              }}
            ></input>
            <label>Name:</label>
            <input
              ...
              value={i_Name}
              onChange={(event) => {
                update_i_Name(event.target.value);
              }}
            ></input>
            ...
            <input
              ...
              value={i_quantity}
              onChange={(event) => {
                update_i_quantity(event.target.value);
              }}
            ></input>
            <label>Price:</label>
            <input
              ...
              value={i_Price}
              onChange={(event) => {
                update_i_Price(event.target.value);
              }}
            ></input>
